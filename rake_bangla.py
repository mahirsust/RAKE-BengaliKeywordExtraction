...
def separate_words(text, min_word_return_size):
    ...
    #splitter = re.compile('[\u002f\u0053\u0056\u00a0\u00ad\u00d0\u00da\u00e6\u00f2\u00f3\u2013\u2014\‘\']')
    
    #splitter = re.compile(u'[৷\u002f\u0053\u0056\u00a0\u00ad\u00d0\u00da\u00e6\u00f2\u00f3\u2013\u2014\\[\\]\n!?,;:\।\t\\"\\(\\)\\\'\‘\‘‘]')
    splitter = re.compile(u'[\u0980-\u09FF]+', re.UNICODE)
    words = re.findall(splitter,text)
    """words = []
    for single_word in splitter.split(text):
        print(single_word)
        currentword = single_word.strip()
        #print(currentword)
        # leave numbers in phrase, but don't count as words, since they tend to invalidate scores of their phrases

        if len(currentword) > min_word_return_size and currentword != '' and not is_number(currentword):
...
def extract_adjoined_candidates(
    sentence_list,
    stoplist,
    min_keywords,
    max_keywords,
    ...
    for s in sentence_list:
        ...
        adjoined_candidates += adjoined_candidates_from_sentence(s,
                stoplist, min_keywords, max_keywords)
...
def adjoined_candidates_from_sentence(
    s,
    stoplist,
    min_keywords,
    max_keywords,
    ...
    for num_keywords in range(min_keywords, max_keywords + 1):
        ...
        for i in range(0, len(sl) - num_keywords):
            ...
            if sl[i] not in stoplist:
                ...
                while keyword_counter < num_keywords and i + j \
                    < len(sl):
                ...
                if contains_stopword and candidate.split()[-1] \
                    not in stoplist and keyword_counter == num_keywords:
...
def generate_candidate_keywords(
    ...
    phrase_list += extract_adjoined_candidates(sentence_list,
            stop_word_list, min_words_length_adj, max_words_length_adj,
            min_phrase_freq_adj)
    return phrase_list
...
class Rake(object):
    ...
    def run(self, text):
        ...
        #print(sentence_list)
        stop_words_pattern = \
            build_stop_word_regex(self.__stop_words_list)
        ...
        keyword_candidates = \
            generate_candidate_keyword_scores(phrase_list, word_scores,
                self.__min_keyword_frequency)

        sorted_keywords = sorted(six.iteritems(keyword_candidates),
...
if test:
    ...
    phraseList = generate_candidate_keywords(sentenceList,
            stopwordpattern, load_stop_words(stoppath))
